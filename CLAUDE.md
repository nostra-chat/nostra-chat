...
**Phase 2b.4 shipped** groups fuzz coverage + 2 real fixes. **Fuzz scaffolding**: 5 actions (`createGroup`, `sendInGroup`, `addMemberToGroup`, `removeMemberFromGroup`, `leaveGroup`), 5 invariants (`INV-group-admin-is-member`, `group-store-unique-ids`, `group-bilateral-membership`, `group-peer-id-deterministic`, `group-no-orphan-mirror-peer`), 7 postconditions, `warmupGroupsHandshake`. 2-user harness + synthetic pubkeys for add/remove; real 3rd browser context → 2b.5. `UserHandle` gained optional `pubkeyHex` (decoded npub→hex **node-side** in harness — bare `'nostr-tools'` does NOT resolve in `page.evaluate`, only Vite-served `/src/...` paths). **Fix #1 (admin-orphan on admin leaveGroup)**: surfaced as 3 INV-group-admin-is-member FINDs (same bug, different groupIds). `handleMemberLeave` removed admin from `members[]` but left `adminPubkey` orphan. Fix in `src/lib/nostra/group-api.ts:handleMemberLeave` — when the leaving member is admin, auto-transfer to the **lex-smallest remaining pubkey**; deterministic, every member derives the same new admin independently, no extra `group_admin_transfer` round required. Unit tests at `src/tests/nostra/group-management.test.ts` (3 new cases). **Fix #2 partial (groups → display bridge, FIND-dbe8fdd2)**: `GroupAPI.onGroupMessage` was declared but never assigned — group messages reached the callback but the dispatch was always null-skipped. Architectural groundwork shipped: `wrapGroupMessage` now returns `{wraps, rumorId}` (2b.3 pattern), `GroupAPI.onOutgoingMessage` TX callback added, `nostra-groups-sync.ts` bridge skeleton with RX+TX handlers, `window.__nostraGroupAPI` singleton to bypass Vite dev multi-instance modules, init wired in `nostra-onboarding-integration.ts`. E2E reproducer at `src/tests/e2e/e2e-groups-bilateral.ts` still FAILs — the end-to-end bubble render requires additional plumbing beyond `nostra_new_message` dispatch (dialogs_multiupdate + invalidateHistoryCache + mirror writes + history_append) which is scoped to **2b.5**. **Baseline v2b4 emit deferred** — blocked by FIND-dbe8fdd2 end-to-end + two 2b.3 fixes re-firing (`FIND-57989db1` mirror/IDB, `FIND-4e18d35d` reaction bilateral). Spec: `docs/superpowers/specs/2026-04-23-bug-fuzzer-phase-2b4-groups-design.md`.
