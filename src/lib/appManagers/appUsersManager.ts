...
export class AppUsersManager extends AppManager {
  ...
  private p2pSyntheticUsers = new Map<number, User>();
  ...
  public getUser(id: User | UserId) {
    ...
    // Check P2P synthetic users first (P2P peer IDs are always numbers)
    if(typeof id === 'number' && this.p2pSyntheticUsers.has(id)) {
      return this.p2pSyntheticUsers.get(id)!;
    }
  ...
  public injectP2PUser(pubkey: string, peerId: number, displayName: string, avatar: string): void {
    ...
    (user as any).p2pPubkey = pubkey;
    (user as any).p2pAvatar = avatar;

    this.users[peerId] = user;
    this.p2pSyntheticUsers.set(peerId, user);

    // Mirror to main thread so apiManagerProxy.getPeer() finds the user
    this.mirrorUser(user);
